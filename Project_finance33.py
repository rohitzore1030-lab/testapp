# -------------------------------------------------------------
# Finance Toolkit — Robust Dark Version (FINAL + LIQUIDITY ADDED)
# -------------------------------------------------------------
...
st.title("📊 Finance Toolkit — Risk, Inflation, Interest, Liquidity & World CPI (Pro Edition)")
...
# -------------------------------------------------------------
# NEW — SAMPLE LIQUIDITY DATA FOR INDIA & USA
# -------------------------------------------------------------
@st.cache_data
def sample_liquidity(country="US", months=120):
    ...
    if country == "US":
        base = 4.0   # Example: Fed reverse repo in trillion $
    else:
        base = 60    # Example: RBI system liquidity in thousand crores
...
# -------------------------------------------------------------
# NEW — LIQUIDITY DASHBOARD (US & INDIA)
# -------------------------------------------------------------
def ui_liquidity():
    st.header("💧 Liquidity Dashboard — USA & India")

    countries = ["US", "IN"]
    uploads = {}

    for c in countries:
        uploads[c] = st.file_uploader(f"Upload Liquidity CSV for {c} (date, liquidity)", type=['csv'], key=f"liq_{c}")
    ...
    for c in countries:
        ...
        if up:
            try:
                tmp = pd.read_csv(up)
                tmp.columns = ['date','Liquidity']
                tmp['date'] = pd.to_datetime(tmp['date'])
                tmp['Liquidity'] = pd.to_numeric(tmp['Liquidity'], errors='coerce')
                df = tmp.dropna()
            except:
                df = sample_liquidity(c)
        else:
            df = sample_liquidity(c)
        ...
        df['Change %'] = df['Liquidity'].pct_change() * 100

        liq_dfs[c] = df

    combined_liq = pd.concat({k: v['Liquidity'] for k, v in liq_dfs.items()}, axis=1)
    st.subheader("System Liquidity Levels")
    st.line_chart(combined_liq.dropna())

    combined_change = pd.concat({k: v['Change %'] for k, v in liq_dfs.items()}, axis=1)
    st.subheader("Liquidity Change (%)")
    st.line_chart(combined_change.dropna())
